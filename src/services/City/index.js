const { createCitys, getCityByid, getAllCity, updateCityByid, deleteCityByid } = require('../City/function');
...
const getCity = async (req, res, next) => {
        getAllCity().then(City => {
            ...
            res.status(500).json({ message: err.message || "Error Occurred while retriving City information" });
            next(err);
        });
};
...
const FindOneCity = async (req, res, next) => {
    ...
    getCityByid(id).then(async City => {
        try {
            if (!City && City.id) {
            ...
            res.status(500).json({ message: "Error retrieving City with id " + id });
        }
...
const updateCity = async (req, res, next) => {
    ...
    const id = req.params.id;
    await updateCityByid(id, data).then(data => {
        if (!data) {
            res.status(404).json({ message: `Cannot Update City with ${id}. Maybe City not found!` });
        } else {
            res.status(200).json({ message: " Successfully Updated City information" });
        }
    }).catch(err => {
...
const deleteCity = async (req, res, next) => {
    const id = req.params.id;
    await deleteCityByid(id).then(data => {
        if (!data) {
