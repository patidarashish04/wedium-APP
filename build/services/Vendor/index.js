...
const getVendor = async (req, res, next) => {
    if (req.query.id) {
        ...
        getVendorByid(id).then(async vendor => {
            try {
                if (!vendor && vendor.id) {
...
const FindOneVendor = async (req, res, next) => {
    ...
    getCategoryByid(id).then(async vendor => {
        try {
            if (!vendor && vendor.id) {
                res.status(404).json({ message: "Not found vendor with id " + id });
            } else {
...
const updateVendor = async (req, res, next) => {
    ...
    const id = req.params.id;
    await updateVendorById(id, data).then(data => {
        if (!data) {
...
const deleteVendor = async (req, res, next) => {
    const id = req.params.id;
    await deleteVendorById(id).then(data => {
        if (!data) {
